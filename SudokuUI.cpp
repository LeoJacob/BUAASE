...
void SudokuUI::updateSudokuBox(int value[81], int rowId, int colId)
{
	...
	if (rowId >= 0 && rowId < NUMBER_OF_ROWS && colId >= 0 && colId < NUMBER_OF_COLUMNS) 
	{
...
void SudokuUI::responseGetTips(int rowId, int colId)
{
	if (ui.pauseButton->isEnabled())
	{
		...
		bool flag = false;
		flag = generateDialog->c.solve(puzzle, solution);			//调用求解数独接口，求得当前状态数独的解
		if (flag)
...
void SudokuUI::receiveQues()
{
	this->ques = generateDialog->result;
	this->iGenerateNumber = generateDialog->iGenerateNumber;
	curQuesNumber = 0;
	switch (generateDialog->iMode)
	{
	case CUSTOM_MODE:
